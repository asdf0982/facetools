...
def face_cropout(srcPath,dstPath,filelist='imageBbox_detect_replace.list',enlarge=True,w=256,h=256):
    ...
    for line in lines:
        ...
        if im.ndim==3:
            ...
            rows,cols = im.shape
        if enlarge==True:
            a1= int(x1-(x2-x1)/2) if (x1-(x2-x1)/2)>=0 else 0
            b1= int(y1-(y2-y1)/2) if (y1-(y2-y1)/2)>=0 else 0 
            a2= int(x2+(x2-x1)/2) if (x2+(x2-x1)/2)<cols else cols
            b2= int(y2+(y2-y1)/2) if (y2+(y2-y1)/2)<rows else rows
        if im.ndim==3:
